...
def diagnostico_e_reparo_tabela_setores():
    """
    Função de emergência para verificar e corrigir a tabela ConfiguracoesSetores.
    Execute uma vez para garantir a estrutura.
    """
    conn = get_db_connection()
    if conn:
        try:
            ...
            print("--- INICIANDO DIAGNÓSTICO DA TABELA ConfiguracoesSetores ---")
            
            # 1. Verifica se a tabela existe
            cursor.execute("SELECT * FROM sys.tables WHERE name = 'ConfiguracoesSetores'")
            if not cursor.fetchone():
                print("ERRO CRÍTICO: Tabela 'ConfiguracoesSetores' NÃO EXISTE. Criando agora...")
                sql_create = """
                    CREATE TABLE ConfiguracoesSetores (
                        Setor VARCHAR(50) PRIMARY KEY,
                        DescricaoPadrao NVARCHAR(MAX)
                ...
                # 2. Verifica se a coluna DescricaoPadrao existe e é NVARCHAR(MAX)
                cursor.execute("""
                    SELECT max_length, is_nullable 
                    FROM sys.columns 
                    WHERE object_id = OBJECT_ID('ConfiguracoesSetores') AND name = 'DescricaoPadrao'
                """)
                col_info = cursor.fetchone()
                
                if not col_info:
                    print("ERRO CRÍTICO: Coluna 'DescricaoPadrao' NÃO EXISTE. Adicionando...")
                    cursor.execute("ALTER TABLE ConfiguracoesSetores ADD DescricaoPadrao NVARCHAR(MAX)")
                    conn.commit()
                    print("SUCESSO: Coluna adicionada.")
                else:
                    # max_length = -1 indica (MAX) para NVARCHAR/VARCHAR
                    print(f"OK: Coluna 'DescricaoPadrao' existe. (Max Length: {col_info[0]})")
                    
                    if col_info[0] != -1: # Se não for MAX, tenta corrigir
                        print("AVISO: Coluna pode ser curta demais. Tentando alterar para NVARCHAR(MAX)...")
                        try:
                            cursor.execute("ALTER TABLE ConfiguracoesSetores ALTER COLUMN DescricaoPadrao NVARCHAR(MAX)")
                            conn.commit()
                            print("SUCESSO: Coluna expandida para MAX.")
                        except Exception as e:
                            print(f"ERRO ao tentar expandir coluna: {e}")
...
# Executa o diagnóstico ao importar (Temporário para debug)
diagnostico_e_reparo_tabela_setores()
