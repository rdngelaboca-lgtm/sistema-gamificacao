...
class AppGestaoEstoque:
    def __init__(self, root):
        ...
        self.mapa_contagens_historico = {} # NOVO: Cache para os dropdowns da Aba 5

        # --- Listener para ATUALIZAR as listas quando as abas mudam ---
        ...
        self.atualizar_lista_produtos() # Atualiza a Aba 1
        self.atualizar_lista_fornecedores() # Atualiza a Aba 2
        self.popular_combobox_produtos_mestre() # Atualiza os combos das Abas 3 e 4
        self.atualizar_lista_contagens_historico() # Atualiza a Aba 4 e o combo da Aba 5
    ...
    def on_tab_changed(self, event):
        ...
        if tab_selecionada == '5. Sugestão de Compra':
            # Atualiza os dropdowns de contagem CADA VEZ que a aba é clicada
            self.popular_combos_contagem_sugestao()

    # ===================================================================
    # == ABA 1: CATÁLOGO MESTRE =========================================
    # ===================================================================
    ...
    def salvar_produto(self):
        ...
        if not nome or not unidade:
            ...
            return
        try:
            estoque_min = Decimal(estoque_min_str) # <-- CORRIGIDO
        except Exception: # Pega erro de conversão
    ...
    # ===================================================================
    # == ABA 3: IMPORTAÇÃO XML (Com Filtro) =============================
    # ===================================================================
    ...
    # --- FUNÇÃO ATUALIZADA (v3) ---
    def popular_combobox_produtos_mestre(self):
        """Atualiza os caches e os comboboxes das Abas 3 e 4."""
        try:
            produtos = database.listar_produtos_estoque()
            
            # Limpa os mapas e listas mestras
            self.mapa_produtos_mestre.clear()
            self.lista_mestre_produtos_nomes.clear()
            
            nomes_produtos_mestre = [] # Lista para o combo da Aba 3
            ...
            # Salva a lista mestra completa para o filtro da Aba 3
            self.lista_mestre_produtos_nomes = sorted(nomes_produtos_mestre)
            # Define os valores iniciais do combo da Aba 3
            if hasattr(self, 'combo_produtos_mestre'):
                self.combo_produtos_mestre['values'] = self.lista_mestre_produtos_nomes
            
            # Atualiza a lista mestra para o filtro da Aba 4
            ...
            if hasattr(self, 'combo_contagem_produtos'):
                # Define os valores iniciais do combo da Aba 4
    ...
    # --- FUNÇÃO ATUALIZADA (v3 - CORRIGIDA) ---
    def ler_xml_nota_fiscal(self, caminho_arquivo_xml):
        """Lê um único arquivo XML e extrai os dados do cabeçalho e dos itens."""
        try:
            parser = ET.XMLParser(remove_blank_text=True)
            tree = ET.parse(caminho_arquivo_xml, parser)
            root = tree.getroot()
            
            # Namespace (essencial para XML de NF-e)
            ns = {'nfe': 'http://www.portalfiscal.inf.br/nfe'}

            # --- Dados do Cabeçalho (Usando findtext) ---
            ide = root.find('.//nfe:ide', ns)
            emit = root.find('.//nfe:emit', ns)
            total = root.find('.//nfe:total/nfe:ICMSTot', ns)
            
            if ide is None or emit is None or total is None:
                raise Exception("Estrutura do XML inválida (tags <ide>, <emit> or <total> não encontradas).")

            dados_nf = {
                'NumeroNF': ide.findtext('nfe:nNF', default='', namespaces=ns),
                'DataEmissao': ide.findtext('nfe:dhEmi', default='T', namespaces=ns).split('T')[0],
                # --- CORREÇÃO BUG 2 (float -> Decimal) ---
                'ValorTotalNF': Decimal(total.findtext('nfe:vNF', default='0.0', namespaces=ns)),
                'FornecedorCNPJ': emit.findtext('nfe:CNPJ', default='', namespaces=ns),
                'FornecedorNome': emit.findtext('nfe:xNome', default='', namespaces=ns)
            }
            
            # --- Dados dos Itens (Usando findtext) ---
            itens = []
            ...
            for prod in produtos_xml:
                ...
                itens.append({
                    'cProd': item_data.findtext('nfe:cProd', default='', namespaces=ns),
                    'cEAN': item_data.findtext('nfe:cEAN', default='', namespaces=ns),
                    'DescricaoXML': item_data.findtext('nfe:xProd', default='', namespaces=ns),
                    'NCM': item_data.findtext('nfe:NCM', default='', namespaces=ns),
                    # --- CORREÇÃO BUG 2 (float -> Decimal) ---
                    'Quantidade': Decimal(item_data.findtext('nfe:qCom', default='0.0', namespaces=ns)),
                    'PrecoCustoUnitario': Decimal(item_data.findtext('nfe:vUnCom', default='0.0', namespaces=ns))
                })
    ...
    # --- FUNÇÃO ATUALIZADA (v2 - Com pop-up de erro) ---
    ...
    def vincular_produto_selecionado(self):
        ...
        if not produto_mestre_selecionado:
            ...
            return
        indice_item = self.tree_vincular.index(selecionado_tree)
        item_pendente = self.itens_xml_nao_vinculados[indice_item]
        ...
        try:
            ...
            self.itens_xml_nao_vinculados.pop(indice_item)
        ...
        try:
            ...
            if not produto_id_mestre:
                ...
                produto_id_mestre = database.criar_produto_estoque(
                    ...
                    unidade="UN", 
                    estoque_min=Decimal('0.0') # <-- CORRIGIDO
                )
                if not produto_id_mestre:
    ...
    # ===================================================================
    # == ABA 4: LANÇAR CONTAGEM FÍSICA (ATUALIZADA com Filtro) ==========
    # ===================================================================
    ...
    def adicionar_item_contagem(self):
        ...
        if not produto_nome or not qtd_str:
            ...
            return
        try:
            quantidade = Decimal(qtd_str) # <-- CORRIGIDO
        except Exception:
    ...
    def salvar_contagem_completa(self):
        ...
        try:
            ...
            if sucesso:
                ...
                self.atualizar_lista_contagens_historico() # Atualiza o histórico (Aba 4 e 5)
            else:
    ...
    def atualizar_lista_contagens_historico(self):
        """Atualiza a lista de histórico na Aba 4 E os combos na Aba 5."""
    ...
    # ===================================================================
    # == ABA 5: SUGESTÃO DE COMPRA (TOTALMENTE REESCRITA) ===============
    # ===================================================================
    def criar_aba_sugestao_compra(self):
        ...
        self.spin_meses_cobertura = ttk.Spinbox(frame_filtros, from_=1, to=12, width=5)
        self.spin_meses_cobertura.set("1") 
        self.spin_meses_cobertura.grid(row=1, column=1, sticky="w", padx=5, pady=5)
        ttk.Label(frame_filtros, text="meses.").pack(side=tk.LEFT)
    ...
    def gerar_sugestao_compra(self):
        ...
        try:
            meses_cobertura = int(self.spin_meses_cobertura.get())
            dias_cobertura = meses_cobertura * 30
            
            # Pega os IDs dos combos
        ...
        try:
            ...
            for item in relatorio_posicao:
                ...
                if umd > 0:
                    estoque_ideal = (umd * Decimal(dias_cobertura)) + minimo
                    sugestao_compra = estoque_ideal - atual
                else:
                    sugestao_compra = Decimal('0.0')
                    
                # Mostra negativo se for o caso
                if sugestao_compra < 0 and atual > minimo:
                    pass # Mantém negativo
                elif sugestao_compra < 0 and atual <= minimo:
                     sugestao_compra = Decimal('0.0') # Se está abaixo do mínimo, não pode ser negativo
    ...
    def popular_combos_contagem_sugestao(self):
        ...
        # Esta função é chamada pela `atualizar_lista_contagens_historico`
        # e pelo listener da aba
        try:
            contagens = database.listar_contagens_cabecalho()
            self.mapa_contagens_historico.clear()
            nomes_contagens = []
