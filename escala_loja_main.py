...
class AppEscalaLoja:
    def __init__(self, root):
        ...
        self.escala_atual = {} # Cache da escala do dia
        self.posicoes = [] # Cache das posições (bolinhas)
        self.mapa_img = None
        self.tk_img = None
        ...
        # Botão de Cadastro Rápido de Freelancer
        ...
        self.canvas.bind("<Button-1>", self.clique_no_mapa) # Clique Esquerdo
        self.canvas.bind("<Button-3>", self.clique_direito_mapa) # Clique Direito (Excluir)

        # Carregar Imagem e Dados Iniciais
        self.carregar_imagem_mapa()
        self.root.after(500, self.carregar_escala_do_dia) # Carrega dados após iniciar

    def carregar_imagem_mapa(self):
        """Carrega a imagem 'layout_loja.png' e ajusta ao canvas."""
        caminho_img = "layout_loja.png"
        if not os.path.exists(caminho_img):
            messagebox.showerror("Erro", f"Imagem '{caminho_img}' não encontrada na pasta!")
            return

        pil_img = Image.open(caminho_img)
        # Redimensiona para caber na tela (ajuste fixo por enquanto, ideal seria dinâmico)
        largura_display = 1080
        altura_display = 600
        pil_img.thumbnail((largura_display, altura_display), Image.Resampling.LANCZOS)
        
        self.tk_img = ImageTk.PhotoImage(pil_img)
        self.canvas.create_image(largura_display/2, altura_display/2, image=self.tk_img, anchor=tk.CENTER)
        self.canvas.config(scrollregion=self.canvas.bbox(tk.ALL))

    def carregar_escala_do_dia(self, event=None):
        """Limpa o mapa e redesenha as bolinhas com base na data selecionada."""
        self.data_selecionada = self.date_entry.get_date().strftime('%Y-%m-%d')
        
        # 1. Buscar Posições (As bolinhas fixas)
        self.posicoes = database.listar_posicoes_loja()
        
        # 2. Buscar Escala do Dia (Quem está onde)
    ...
    def redesenhar_marcadores(self):
        """Desenha as bolinhas no mapa. Verde = Ocupado, Vermelho = Vazio."""
        self.canvas.delete("marcador") # Limpa marcadores anteriores
        ...
        for pos in self.posicoes:
            ...
            # Verifica se já tem alguém escalado manualmente
            if pos_id in self.escala_atual:
                dados_escala = self.escala_atual[pos_id]
                nome_pessoa = dados_escala.NomePessoa
                ocupado = True
                cor = "#00C851" # Verde (Escalado)
            
            # Se vazio, verifica se tem um FUNCIONÁRIO PADRÃO para essa posição
    ...
    def clique_no_mapa(self, event):
        ...
        if self.modo_edicao:
            # Modo Configuração: Cria nova posição
            nome = simpledialog.askstring("Nova Posição", "Nome do local (ex: Caixa 1):")
            if nome:
                database.criar_posicao_loja(nome, x, y)
                self.carregar_escala_do_dia() # Recarrega
        else:
            # Modo Escalação: Verifica se clicou em uma bolinha existente
            # Procura itens próximos ao clique
    ...
    # --- JANELA DE ESCALAÇÃO (O Coração do Sistema) ---
    def abrir_janela_escalacao(self, pos_id):
        # Busca nome da posição
        ...
        for f in database.listar_funcionarios():
            ...
            mapa_ids[label] = {'tipo': 'func', 'id': f.FuncionarioID, 'tel': f.ChatIDTelegram} # ChatID não é tel, mas ok
        ...
        # Preenche se já tiver alguém
        pessoa_selecionada_tel = None
        if dados_atuais:
            if dados_atuais.FuncionarioID:
                # Busca o nome na lista que contém [Fixo]
                nome_match = next((k for k, v in mapa_ids.items() if v['tipo'] == 'func' and v['id'] == dados_atuais.FuncionarioID), "")
                combo_pessoas.set(nome_match)
            elif dados_atuais.FreelancerID:
                nome_match = next((k for k, v in mapa_ids.items() if v['tipo'] == 'free' and v['id'] == dados_atuais.FreelancerID), "")
                combo_pessoas.set(nome_match)
        ...
        def salvar():
            ...
            if not selecao or selecao == "(Vazio)":
                # Lógica para limpar a escala (implementar update com NULLs se desejar limpar)
                return 
            ...
            if sucesso:
                messagebox.showinfo("Sucesso", "Escala salva!")
                popup.destroy()
                self.carregar_escala_do_dia()
            else:
        ...
        def enviar_zap():
            ...
            # Se for freelancer e tiver telefone
            telefone = dados_pessoa.get('tel')
            if not telefone:
                # Tenta buscar do banco se já estava salvo antes
                if pessoa_selecionada_tel: telefone = pessoa_selecionada_tel
